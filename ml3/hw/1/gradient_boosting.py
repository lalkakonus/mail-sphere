...
class GradientBoostingClassifier:
    ...
    def fit(self, X_train: np.array, y_train: np.array, X_test=None, y_test=None):
        ...
        depth = 1
        #last_increase = 0
        bar = progressbar.ProgressBar(max_value=self.n_estimators)
        for i in range(self.n_estimators):
            ...
            self.add_estimator(X_train, y_train, depth, i)
        return self
    ...
    def lr(self, i):
        ...
        return min(
            ...
            self.learning_rate + (self.min_learning_rate - self.learning_rate) / self.n_estimators * i)

    def staged_predict_proba(self, X):
        n_samples = X.shape[0]
        result = np.zeros([self.n_estimators, n_samples])
        result[0] = self.lr(0) * self.estimators_array[0].predict(X).reshape(-1)
        result[0] += self.lr(0) * self.estimators_array[1].predict(X).reshape(-1)
        
        for i, estimator in enumerate(self.estimators_array[2:], 1):
            result[i] = result[i - 1] + self.lr(i) * estimator.predict(X).reshape(-1)
    ...
    def initialize_model(self, X, y):
        model = ConstModel().fit(math.log(y.sum() / (y.size - y.sum())))
    ...
    def add_estimator(self, X, y, depth, i):
        ...
        model = DecisionTreeRegressor(
            ...
            max_features=self.max_features).fit(X, target, self.last_prediction)

        self.last_margin = self.last_margin + self.lr(i) * model.predict(X).reshape(-1)
